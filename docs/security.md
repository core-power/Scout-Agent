...
- 换行符 `\n` / `\r` 拦截（禁止多行命令走私）
- 路径遍历 `..` 风险分级（2026-09-21 放宽，见 2.3.1）
...
### 2.3.1 相对上级路径（`..`）放宽策略（2026-09-21）

旧规则「只要出现 `..` 独立段就拒绝」会误杀大量日常本地操作
（`cd ..\上层目录`、`dir ..\兄弟目录`、`type ..\配置.ini`、`cat ../README.md`）。
相对上级路径本身不构成越权，因此改为**按风险分级**，只硬拦截明显危险的写法：

| 类别 | 触发条件 | 示例 |
|---|---|---|
| A | 落点命中 Windows 系统关键目录（任意深度） | `type ..\..\Windows\win.ini` |
| B | 上跳 ≥2 级后首个落点是 Unix 系统目录 | `cat ../../etc/passwd` |
| C | 连续上跳 ≥4 级的病态深逃逸 | `cd ..\..\..\..\..` |
| D | 破坏性命令（del/rm/rmdir/move…）配合 `..`：越级（≥2 级）/ 通配符 / 裸 `..` | `rm -rf ..`、`del ..\*.*` |

其余相对上级路径（读取、进入、复制、上跳 1 级的明确删除目标）一律放行；
cwd 白名单与 `SYSTEM_DIRS` 绝对路径拦截不变。

### 2.3.2 权限开关：风险分级 × 用户授权（2026-09-21）

安全策略分**两个维度**，二者相乘决定一条命令的最终去向：

**维度一 · 风险分级**（`scout/security/policy.py`）

| 级别 | 含义 | 示例 |
|---|---|---|
| `never` | 不可逆 / 不可审计 | `rm -rf /`、`mkfs`、`dd if=`、`curl \| sh`、fork 炸弹、注入载荷 |
| `risky` | 高危但用户可判断后果 | `rm -rf build`、`del /f /s 旧目录`、`rmdir /s /q ..`、`shutdown`、`kill -9`、`git reset --hard`、读 `~/.ssh/id_rsa` |
| `normal` | 常规操作 | `dir`、`type a.txt`、`git status` |

**维度二 · 权限开关**（输入框左侧「权限」下拉，持久化到 `config.permission_mode`）

| 模式 | 行为 |
|---|---|
| `ask`（标准，默认） | `risky` 弹窗询问，批准后才执行；`normal` 直接执行；`never` 仍拦截 |
| `auto`（全部放行） | `risky` 也不再询问，直接执行（仅留痕 `tool.risky_auto`）；`never` 仍拦截 |
| `strict`（谨慎） | 每条 shell / 代码执行都先确认 |

接线：`_gate_security_checks` 只拦 `never`；`_gate_permission` 按模式决定是否
`on_confirm` 弹窗，批准后向工具参数注入 `_approved=True`，`_validate_command(approved=True)`
据此放行高危项（`never` 与结构性拦截不受影响）。已在权限门控问过的调用不再触发 HITL 二次询问。
