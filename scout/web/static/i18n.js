...
const I18N_DICT = {
    ...
    "⚠ 图片不会被识读": { zh: "⚠ 图片不会被识读", en: "⚠ Images will not be read" },
    "去设置": { zh: "去设置", en: "Open settings" },
    "图片将直接发给当前模型": { zh: "图片将直接发给当前模型", en: "Images go straight to the current model" },
    "图片已转成文字描述": { zh: "图片已转成文字描述", en: "Images were converted to text descriptions" },
    "当前模型不直接看图，将由": { zh: "当前模型不直接看图，将由", en: "This model can't see images directly —" },
    "当前配置无可用视觉模型": { zh: "当前配置无可用视觉模型", en: "no vision model is available in the current config" },
    "未送达": { zh: "未送达", en: "Not delivered" },
    "视觉模型": { zh: "视觉模型", en: "Vision model" },
    "识图成文字": { zh: "识图成文字", en: "will describe them as text instead" },
    "部分图片未识读": { zh: "部分图片未识读", en: "Some images were not read" },

    "主模型不收图（交视觉模型识图）": { zh: "主模型不收图（交视觉模型识图）", en: "Main model won't take images (use a vision model)" },
    "关闭视觉处理": { zh: "关闭视觉处理", en: "Disable vision" },
    "填成与主模型一样的名字 = 声明「主模型自己能看图」，图片将直接发给它（自定义模型最常用）。": { zh: "填成与主模型一样的名字 = 声明「主模型自己能看图」，图片将直接发给它（自定义模型最常用）。", en: "Enter the same name as the main model to declare native vision — images go straight to it (most common choice for custom models)." },
    "实测不收图片": { zh: "实测不收图片", en: "Tested: rejects images" },
    "实测能否看图": { zh: "实测能否看图", en: "Test vision" },
    "实测能看图": { zh: "实测能看图", en: "Tested: can see images" },
    "已实测：不收图片": { zh: "已实测：不收图片", en: "Tested: rejects images" },
    "已实测：能看图": { zh: "已实测：能看图", en: "Tested: can see images" },
    "探测中…": { zh: "探测中…", en: "Testing…" },
    "探测失败": { zh: "探测失败", en: "Vision test failed" },
    "探测请求失败": { zh: "探测请求失败", en: "Vision test request failed" },
    "无法判定": { zh: "无法判定", en: "Cannot determine" },
    "暂无 Webhook": { zh: "暂无 Webhook", en: "No webhooks" },
    "未能证实（疑似网关丢弃图片）": { zh: "未能证实（疑似网关丢弃图片）", en: "Unverified (the gateway may drop images silently)" },
    "端点/网络异常，无法判定": { zh: "端点/网络异常，无法判定", en: "Endpoint/network error — cannot determine" },
    "缺少 Pillow，无法生成测试图": { zh: "缺少 Pillow，无法生成测试图", en: "Pillow missing — cannot build the test image" },
    "能力靠猜，建议实测一次": { zh: "能力靠猜，建议实测一次", en: "Capability is guessed here — run the test once" },
    "能看图（主模型直收）": { zh: "能看图（主模型直收）", en: "Can see images (sent to the main model)" },
    "请先选择或填写模型": { zh: "请先选择或填写模型", en: "Select or enter a model first" },
    "鉴权失败，无法判定": { zh: "鉴权失败，无法判定", en: "Auth failed — cannot determine" },
    "（未记录，可修正配置后重试）": { zh: "（未记录，可修正配置后重试）", en: "(not recorded — fix the config and retry)" },

    "」吗？\n将删除 $SCOUT_DATA_DIR/skills/": { zh: "」吗？\n将删除 $SCOUT_DATA_DIR/skills/", en: "」? \nwill delete $SCOUT_DATA_DIR/skills/" },
    "将删除 $SCOUT_DATA_DIR/skills/": { zh: "将删除 $SCOUT_DATA_DIR/skills/", en: "will delete $SCOUT_DATA_DIR/skills/" },
