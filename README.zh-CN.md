...
| 5 | 🤝 **Agent 与 Agent 互通（A2A）** | 实现 Google A2A 协议——别的智能体可以把任务派给 Scout，Scout 也可以派给别人。面向多智能体未来，而不是一座孤岛。 |
| 6 | 🔒 **天生安全优先** | Docker 沙箱执行、危险命令黑名单（`rm -rf /`、fork 炸弹……）、Shell 注入与 XSS 防护、可选 JWT 认证、密钥加密存储。让 Agent 执行代码很可怕——Scout 让它变得无感。 |
| 7 | ⚡ **零门槛上手** | Windows 绿色版：解压 → 双击 → 完事（免 Python、免安装、免注册表）；也可以直接克隆仓库跑。数据随程序所在盘符存放，拷贝到别的电脑接着用。 |
...
- 内置 **20+ 工具**：文件编辑、安全 Shell、代码执行、联网搜索、记忆回溯、定时任务、MCP 接入等，按需授予；
...
│   ├── tools/                # 工具实现
│   │   └── builtin/          # 20+ 内置工具（文件/shell/代码执行/搜索/MCP/浏览器…）
│   ├── voice/                # 语音（ASR/TTS）
