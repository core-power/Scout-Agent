...
| 5 | 🖱 **真的长着"手"** | 内置 `desktop` 桌面操控工具：真实点击/打字/截图，直接替你操作**本机任意软件**——微信发消息、飞书找人、浏览器导航、Excel 填表。附带 10 个真机实测技能库（`skills-library/`），说"给XX发条消息"它就真去发。 |
| 6 | 🤝 **Agent 与 Agent 互通（A2A）** | 实现 Google A2A 协议——别的智能体可以把任务派给 Scout，Scout 也可以派给别人。面向多智能体未来，而不是一座孤岛。 |
| 7 | 🔒 **天生安全优先** | Docker 沙箱执行、危险命令黑名单（`rm -rf /`、fork 炸弹……）、Shell 注入与 XSS 防护、可选 JWT 认证、密钥加密存储。让 Agent 执行代码很可怕——Scout 让它变得无感。 |
| 8 | ⚡ **零门槛上手** | Windows 绿色版：解压 → 双击 → 完事（免 Python、免安装、免注册表）；也可以直接克隆仓库跑。数据随程序所在盘符存放，拷贝到别的电脑接着用。 |

**最适合这些场景：** 一个帮你跑日常杂活的个人副驾——定时 + 事件驱动的自动化任务、监听文件夹与 Webhook、基于私有知识库问答、通过团队 IM 汇报结果——同时密钥、记忆、代码都私有地留在你自己的硬件上。

> 👉 想看完整清单？见下方 [特性](#特性) 表（附真实界面截图），或直接跳到 [快速开始](#快速开始)。
...
下面 5 个是 Scout 区别于普通 Chatbot 的**底层机制**——也是"为什么越用越强"的答案。界面级功能（自动化中心、观测时间线、事件总线、通知、文件监听、外部回调等）见下方「网页界面」章节。
